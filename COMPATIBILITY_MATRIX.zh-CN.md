...
| Windows 版本 | Windows 11 Home 10.0.26200（构建 26200） | 实体通过 | 使用 `Win32_OperatingSystem` 采集当前主机版本；已手动启动启动器和悬浮窗。 |
| 显示器 | 单显示器 | 自动化+部分实体 | 几何 API 测试通过；仍需保存单显示器实体运行截图。 |
| 显示器 | 双显示器 | 实体通过 | 已完成当前 `DISPLAY1` 和 `DISPLAY2` 探测；工作区为 `0,0-2048,1104` 和 `2560,354-4480,1386`；副屏坐标 `(4150,1248)` 可显示悬浮窗。 |
| 坐标 | 虚拟桌面负坐标 | 自动化 | `Display API` 相交和定位测试覆盖负坐标。 |
| 坐标 | 大副屏坐标 `(4151,1248)` | 实体通过 | 已在副屏观察到悬浮窗和右键菜单。 |
| 菜单 | 四角和任务栏工作区 | 自动化+部分实体 | 几何测试通过；任务栏不同边缘的实体矩阵仍待完成。 |
...
| DPI | 100% / 125% / 150% / 200% | 自动化部分完成 | 模拟 96/120/144/192 DPI 路径通过；实体混合 DPI 测试仍待完成。 |
...
| 启动器 | 根目录 `start_codex_status_pet.cmd`，重复启动 | 实体通过 | 连续启动两次只产生一个 `pythonw.exe` 悬浮窗进程，且没有控制台窗口。 |
