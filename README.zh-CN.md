...
`CodexStatusPet.exe`。请保留 EXE 旁的 `_internal` 目录；此直接使用方式不创建快捷方式，
也不安装或修改全局 Python。公开 bootstrap 仍是安装最新版、修复和升级时推荐的
自动 SHA-256 校验方式；手动 ZIP 用户可核对发布的 `.sha256` sidecar。

### Source 开发者包

只有需要源码包的开发者才应使用 `-Channel Source`。它与 Standalone 使用同一应用版本，
但要求 x64 Python 3.10+、Tkinter、pip 和 `pythonw.exe`；不会自动安装 Python 或修改 PATH：
...
Source ZIP 名称为 `CodexStatusPet-v1.1.0-source-win11-x64.zip`，并保留 `launch.cmd` 用于排错。
