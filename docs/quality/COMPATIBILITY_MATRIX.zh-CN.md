...
## v0.9.2-beta.1 Signal HUD 候选版本

| 范围 | 覆盖内容 | 状态 | 证据 / 下一步 |
|---|---|---|---|
| v0.9.2-beta.1 UI 回归 | 展开四行 HUD、收缩百分比电池、统一额度文字颜色、进度条颜色、设置预览和菜单 | Windows 主机自动化通过 | 隔离分支上的质量 UI 套件和 README 截图检查通过。 |
| v0.9.2-beta.1 Shell 生命周期 | 设置保存/关闭、窗口管理器关闭、置顶恢复、延迟刷新及主界面右键菜单打开/关闭/操作路径 | HWND 生命周期自动化通过；实体观察待补 | 菜单路径现在会刷新弹窗和主窗口 HWND。在接受其替代 `main` 成为 1.0 前，仍需重新进行实体任务栏观察；beta 分支不声称已有该证据。 |
