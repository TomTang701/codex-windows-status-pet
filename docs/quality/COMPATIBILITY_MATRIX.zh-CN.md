...
| v0.4.1 内容与错误 | 全部 25 个缩放档位；托盘/额度失败路径 | **阻塞 — 裁剪测试已失效** | [2026-07-11 v0.4.1正确性记录](test-records/2026-07-11-v0.4.1-correctness-validation.md)仅保留为假阳性证据。真实生产截图仍显示 Reset Credit 被裁剪；必须完成 DPI 忠实重现和替代契约。错误路径集成仍为绿色，但不能解除布局阻塞。 |
