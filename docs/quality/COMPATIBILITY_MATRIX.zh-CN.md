...
| v0.4.1 内容与错误 | 全部 25 个缩放档位；托盘/额度失败路径 | Windows 主机和集成通过 | [2026-07-11 v0.4.1正确性记录](test-records/2026-07-11-v0.4.1-correctness-validation.md)；映射的真实/请求几何可容纳全部五行，已移除旧式 Label 调用，批准的错误行正常渲染，不显示原始异常文本，托盘恢复只调度一次。 |
