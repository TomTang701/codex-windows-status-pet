...
Signal HUD beta 的展开视图最多渲染四行：活动进度、5 小时额度、每周额度和重置额度；底层五个稳定身份不变，活动身份由标题/状态区域承载。收缩视图只渲染带百分比的十格电池。
...
- 映射、置顶、透明度、设置窗口打开/关闭及延迟关闭回调之后，真实 Tk 根窗口必须保持 `WS_EX_TOOLWINDOW` 且不带 `WS_EX_APPWINDOW`；Shell 身份规范化由 UI 生命周期测试验证。
