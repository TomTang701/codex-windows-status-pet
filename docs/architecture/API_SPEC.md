...
| Window Scale API | `scripts/api/window_scale_api.py` | Clamp/quantize one canonical percentage, derive immutable geometry/typography/layout metrics with content-safe vertical padding at measured clipping scales, and infer scale from legacy visual area. | All 25 scale steps, mapped content fit, half-up steps, monotonicity, ratio tolerance, malformed values, and migration bounds. |
...
- Coordinates may be negative; canonical scale is 80–200% in 5% steps; derived expanded geometry remains 264x110 through 660x276; measured content-safe vertical padding is 7 px at 80% and 8 px at 95%; refresh interval is clamped to 1–10 seconds.
